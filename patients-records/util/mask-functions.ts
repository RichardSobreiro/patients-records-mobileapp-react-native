...
export const maskCreditCardNumber = (inputCard: string): string => {
  const cardValue = inputCard.replace(/\D/g, '').match(/(\d{0,4})(\d{0,4})(\d{0,4})(\d{0,4})/);
  if (cardValue) {
...
export const maskCPF = (inputCard: string): string => {
  const cardValue = inputCard.replace(/\D/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,3})(\d{0,2})/);
  if (cardValue) {
    inputCard = !cardValue[2]
      ? cardValue[1]
      : `${cardValue[1]}.${cardValue[2]}${`${cardValue[3] ? `.${cardValue[3]}` : ''}`}${`${
...
export const maskMobilePhoneNumber = (inputPhoneNumber: string): string => {
  const values = inputPhoneNumber.replace(/\D/g, '').match(/(\d{0,2})(\d{0,1})(\d{0,4})(\d{0,4})/);
  if (values) {
...
export const maskCEP = (inputCard: string): string => {
  ...
  if (cardValue) {
    inputCard = !cardValue[2] ? cardValue[1] : `${cardValue[1]}-${cardValue[2]}`;
  }
  return inputCard;
};
